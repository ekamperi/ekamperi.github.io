...
An intuitive approach to defend against against adversarial attacks is *adversarial training*, i.e. the addition of adversarial examples into the training set during the training process. The first to come up with this defense was Goodfellow et al. who proposed to increase the robustness of the model by injecting both original (non-adversarial) and adversarial examples generated by the fast gradient sign method. The modified objective function would then be:

$$
\begin{align}
\underbrace{J'(x, y_\text{true})}_{\text{new cost function}}
&= \lambda \underbrace{J(x, y_\text{true})}_{\substack{\text{non-adversarial}\\ \text{examples}}} + (1-\lambda) \underbrace{J(x_\text{adv}, y_\text{true})}_{\text{adversarial examples}}\\
