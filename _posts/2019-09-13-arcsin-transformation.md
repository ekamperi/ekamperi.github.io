...
The reason behind this failure is, I guess, that my data were noisy and this transformation *inflated* the error. You can check this wikipedia article on [propagation of uncertainty](https://en.wikipedia.org/wiki/Propagation_of_uncertainty). A common formula to calculate error propagation is the following. Assuming that you have $$z = f(x, y, \ldots)$$, then your error in the variable $$z$$ is given by:
...
Excercise for the reader: What data transformation would actually reduce my error? Can you think of some function $$z = f(y)$$, such as that when you calculate $$\frac{dz}{dy}$$, $$s_z$$ is smaller compared to $$s_y$$ for $$y$$ values close to $$1$$?
