...
Every $$\ell_p$$ attaches a different "size" in vectors and the question on what's the best norm to use, depends on the problem you are solving. For example, if you are building an application for taxi drivers in Manhattan that needs the minimal distance between two places, then using the $$\ell_1$$ norm would make more sense than $$\ell_2$$.
