...
Feel free to check the [PCDimension](https://cran.r-project.org/web/packages/PCDimension/PCDimension.pdf) *R* package on how to estimate the number of significant components. One technique is to "broken stick" model. The idea behind this is to model $$N$$ variances by assuming a stick of unit length and breaking it into $$N$$ pieces by randomly (and simultaneously) selecting break points from a uniform distribution. We then compare element-wise the percentage variances of our components against the percentages from the broken stick distribution. The first value $$d+1$$ where the expected is larger than the observed is the target dimension. Another method is to use the bootstrap resampling technique to calculate confidence intervals for the eigenvalues and keep those whose CI contains 1.
