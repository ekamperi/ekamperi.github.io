...
6. One very important issue is that of **interpretability**. Once we have replaced our original variables with the principal components, it's not always entirely trivial to interpret the results.
    ...
    + Another path to simplification, therefore to interpretation, of PCs is to force additional constraints on the new variables. One such method is LASSO, which we [already discussed](https://ekamperi.github.io/machine%20learning/2019/10/19/norms-in-machine-learning.html#regularization) in the context of regularization.
