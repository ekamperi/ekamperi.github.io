...
6. One very important issue is that of **interpretability**. Once we have replaced our original variables with the principal components, it's not always entirely trivial to interpret the results.
    ...
    + Another path to simplifying PCs, therefore to interpretation, is to force additional constraints on the new variables, e.g., an $$L_1$$ constraint. Another way is to refortmulate PCA as a regression problem and use LASSO, which we [already discussed](https://ekamperi.github.io/machine%20learning/2019/10/19/norms-in-machine-learning.html#regularization) in the context of regularization. Either way, that's the field of [Sparse PCA](https://web.stanford.edu/~hastie/Papers/spc_jcgs.pdf).
