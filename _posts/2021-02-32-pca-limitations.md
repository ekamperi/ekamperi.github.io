...
0. In order to apply PCA and produce meaningful results, we must first check whether some assumptions hold, like the presence of linear correlations or sampling adequacy. This step is often overlooked. 
...
5. One very important issue is that of **interpretability**. Once we have replaced our original variables with the principal components, it's not always entirely trivial to interpret thr results. Sometimes, depending again on data's structure, one might apply a rotation *after PCA* to simplify the interpretation of the components. Such rotations include Varimax and oblique rotations. 
