...
0. In order to apply PCA and produce meaningful results, we must first **check whether some assumptions hold**, like the presence of linear correlations (e.g. Bartlett's test of sohericity) or sampling adequacy (e.g., Kaiser-Meyer-Olkin test). This step is often overlooked, and strictly speaking is not a limitation of PCA, but of the person running the analysis. 
