...
6. One very important issue is that of **interpretability**. Once we have replaced our original variables with the principal components, it's not always entirely trivial to interpret the results.
    ...
    + Another path to simplifying PCs, therefore to interpretation, is to force additional constraints on the new variables. One such method is LASSO, which we [already discussed](https://ekamperi.github.io/machine%20learning/2019/10/19/norms-in-machine-learning.html#regularization) in the context of regularization. That's the field of [Sparse PCA](https://web.stanford.edu/~hastie/Papers/spc_jcgs.pdf).
