...
Last, the so called *angle condition* needs to be satisfied. The intuition is that we can't be moving nearnly orthogonal to the direction of gradient descent. Therefore we need the $$\cos(\mathbf{u}, \mathbf{v})$$ with $$u = -\mathbf{H}^{-1}f(\mathbf{x})$$ and $$ v = -\nabla_x f(\mathbf{x})$$ to satisfy the condition $$\cos(\mathbf{u}, \mathbf{v}) \ge \epsilon > 0 \leftrightarrow \frac{\mathbf{u} \mathbf{v}}{\left\| \mathbf{u} \cdot \mathbf{v} \right\|} \ge \epsilon$$. 
