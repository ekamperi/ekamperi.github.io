...
Last, the so called *angle condition* needs to be satisfied. The intuition is that we can't be moving nearly orthogonal to the direction of gradient descent. Therefore we need the $$\cos(\mathbf{u}, \mathbf{v})$$ with $$\mathbf{u} = \mathbf{H}^{-1}f(\mathbf{x})$$ and $$ \mathbf{v} = -\nabla_x f(\mathbf{x})$$ to satisfy the condition $$\cos(\mathbf{u}, \mathbf{v}) \ge \epsilon > 0 \Leftrightarrow \frac{\mathbf{u} \cdot \mathbf{v}}{\left\| \mathbf{u} \cdot \mathbf{v} \right\|} \ge \epsilon$$. 
